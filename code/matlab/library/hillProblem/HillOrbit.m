classdef HillOrbit < matlab.mixin.Copyable
    ...
    methods
        function obj = HillOrbit(x0, System)
            if(nargin ==2)
                obj.System = System;
                obj.x0 = x0;
                obj.x = obj.x0;
        ...
        function xInertial = inertialMotion(obj, t, x)
            ...
            for i = 1:numel(theta)
                % add velocity from rotation
                if(size(x,1) == 4)
                    x(:,i) = x(:,i) + [0; 0; -x(2,i); x(1,i)];
                else
                    x(:,i) = x(:,i) + [0; 0; 0; cross([0;0;1], x(1:3,i))];
                end
        ...
        function h = plot(obj, textZVC)
            ...
            if(nDim == 2)
                ...
                title(['Motion in the rotating frame for ' num2str(timeVal) units])
                xlabel('x [non-dimensional units]')
                ylabel('y [non-dimensional units]')
            else
                
                [h(1), ~] = obj.System.plotSystem3d();
